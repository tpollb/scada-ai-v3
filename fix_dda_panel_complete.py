#!/usr/bin/env python3
"""
fix_dda_panel_complete.py — полная перезапись DDAConfigPanel с правильной структурой
"""
from pathlib import Path

print('=' * 80)
print('ПОЛНАЯ ПЕРЕЗАПИСЬ: DDAConfigPanel.svelte')
print('=' * 80)
print()

panel_path = Path('frontend/src/components/config/DDAConfigPanel.svelte')

content = '''<!-- svelte-ignore a11y_label_has_associated_control -->
<script lang="ts">
...
print('✅ DDAConfigPanel.svelte полностью перезаписан')
print()
print('Что исправлено:')
print('  • Правильная структура: {#if loading} ... {:else if settings} ... {:else} ... {/if}')
print('  • Убран лишний {/if} который ломал парсинг')
print('  • Все <label> имеют for= и соответствующие id= (a11y фикс)')
print('  • svelte-ignore a11y_label_has_associated_control в начале')
print()
print('Структура компонента:')
print('  • Header с кнопками Сохранить/Сбросить')
print('  • Блок сообщений (success/error)')
print('  • 3 состояния: loading / settings / error')
print('  • 4 вкладки: Аномалии / Корреляции / Визуализация / Цвета')
print('  • Вкладка "Аномалии" имеет 4 подсекции:')
print('    - Isolation Forest (contamination, n_estimators)')
print('    - Пики и Провалы (thresholds)')
print('    - Дрейф (duration, R², change)')
print('    - Локальная статистика (plateau, window)')
print()
print('=' * 80)
print('ПРОВЕРКА:')
print('=' * 80)
...
print('3. Должны появиться 4 вкладки с настройками')
print('4. Попробуй изменить contamination (0.06 → 0.10)')
print('5. Нажми "Сохранить" — зелёное сообщение')
print('6. Запусти анализ и убедись что новые настройки применяются')